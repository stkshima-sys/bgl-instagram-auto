...
import { env, todayJST, nowJST, Slot } from "../config/env.js";
import {
...
/**
 * queue/carousel/今日_スロット.json を読み、rawURL経由でInstagramにカルーセル投稿する。
 * 前提: 画像がGitHubにpush済みで MEDIA_BASE_URL から取得できること。
 */
export async function postCarousel(slot: Slot): Promise<void> {
  const date = todayJST();
  const queuePath = join("queue/carousel", `${date}_${slot}.json`);
  ...
  console.log(`✅ カルーセル投稿完了 [${slot}]: ${permalink}`);
  ...
  renameSync(queuePath, join("posted/carousel", `${date}_${slot}.json`));
}
